...
public class StudentController extends ToolController {
    ...
    public void initialize() {
        ...
        // 学生登录时，如果只有一条数据，自动选中自己的记录
        if ("ROLE_STUDENT".equals(AppStore.getJwt().getRole()) && !observableList.isEmpty()) {
    ...
    protected void onSaveButtonClick() {
        ...
        String loginRole = AppStore.getJwt().getRole();

        // 学生只能修改自己的信息
        if ("ROLE_STUDENT".equals(loginRole)) {
            if (!loginUser.equals(numField.getText())) {
                MessageDialog.showDialog("学生只能修改自己的信息！");
                return;
            }
        }
