...
public class StudentStatisticsService {
    ...
    public StudentStatisticsService(StudentRepository studentRepository, ScoreRepository scoreRepository, HomeworkSubmissionRepository homeworkSubmissionRepository, StudentLeaveRepository studentLeaveRepository, StudentStatisticsRepository studentStatisticsRepository) {
    ...
    public DataResponse getStudentStatisticsList(DataRequest dataRequest) {
        ...
        for (StudentStatistics ss : sList) {
            ...
            m.put("gpa", ss.getGpa());
            m.put("homeworkCount", ss.getHomeworkCount() == null ? "0" : ss.getHomeworkCount() + "");
            m.put("homeworkAvgScore", ss.getHomeworkAvgScore() == null ? 0d : ss.getHomeworkAvgScore());
            m.put("no", ss.getNo()+"");
    ...
    public DataResponse doStudentStatistics(DataRequest dataRequest) {
        ...
        for(i = 0; i < sList.size(); i++) {
            ...
            ss.setGpa(0d);
            ss.setAvgScore(0d);
            ss.setHomeworkCount(0);
            ss.setHomeworkAvgScore(0d);
            ss.setLeaveCount(0);
        ...
        Map<Integer, Double> homeworkScoreSumMap = new HashMap<>();
        Map<Integer, Integer> homeworkCountMap = new HashMap<>();
        List<HomeworkSubmission> homeworkSubmissions = homeworkSubmissionRepository.findGradedByStudentIds(idList);
        if(homeworkSubmissions != null && !homeworkSubmissions.isEmpty()) {
            for(HomeworkSubmission submission : homeworkSubmissions) {
                if(submission.getStudent() == null || submission.getGrade() == null)
                    continue;
                personId = submission.getStudent().getPersonId();
                double totalScore = submission.getAssignment() == null || submission.getAssignment().getTotalScore() == null
                        ? 100d
                        : submission.getAssignment().getTotalScore();
                if(totalScore <= 0)
                    totalScore = 100d;
                double percentScore = submission.getGrade() * 100d / totalScore;
                homeworkScoreSumMap.put(personId, homeworkScoreSumMap.getOrDefault(personId, 0d) + percentScore);
                homeworkCountMap.put(personId, homeworkCountMap.getOrDefault(personId, 0) + 1);
            }
            for(Map.Entry<Integer, Integer> entry : homeworkCountMap.entrySet()) {
                personId = entry.getKey();
                ss = sMap.get(personId);
                if(ss == null)
                    continue;
                int homeworkCount = entry.getValue();
                double homeworkSum = homeworkScoreSumMap.getOrDefault(personId, 0d);
                double homeworkAvg = homeworkCount == 0 ? 0d : homeworkSum / homeworkCount;
                ss.setHomeworkCount(homeworkCount);
                ss.setHomeworkAvgScore(CommonMethod.getDouble2(homeworkAvg));
                int courseCount = ss.getCourseCount() == null ? 0 : ss.getCourseCount();
                double courseAvg = ss.getAvgScore() == null ? 0d : ss.getAvgScore();
                int totalCount = courseCount + homeworkCount;
                if(totalCount > 0) {
                    double combinedAvg = (courseAvg * courseCount + homeworkSum) / totalCount;
                    ss.setAvgScore(CommonMethod.getDouble2(combinedAvg));
                    ss.setGpa(CommonMethod.getDouble2(combinedAvg / 20d));
                }
            }
        }
    ...
    private void resetStatistics(StudentStatistics ss) {
        ss.setCourseCount(0);
        ss.setGpa(0d);
        ss.setAvgScore(0d);
        ss.setHomeworkCount(0);
