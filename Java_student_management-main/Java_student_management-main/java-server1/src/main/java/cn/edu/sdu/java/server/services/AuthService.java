...
public class AuthService {
    ...
    public DataResponse getSwitchAccountList(DataRequest dataRequest) {
        String roleType = dataRequest.getString("roleType");
        String roleName = mapRoleTypeToAuthority(roleType);
        if (roleName == null) {
            return CommonMethod.getReturnData(new ArrayList<>());
        }
        List<User> userList = userRepository.findByUserTypeNameOrderByUserName(roleName);
        List<Map<String, Object>> resultList = new ArrayList<>();
        for (User user : userList) {
            Map<String, Object> item = new HashMap<>();
            item.put("id", user.getPersonId());
            item.put("value", user.getUserName());
            String personName = user.getPerson() == null ? "" : user.getPerson().getName();
            item.put("title", user.getUserName() + (personName == null || personName.isEmpty() ? "" : " - " + personName));
            resultList.add(item);
        }
        return CommonMethod.getReturnData(resultList);
    }

    public ResponseEntity<?> switchAccount(DataRequest dataRequest) {
        String username = dataRequest.getString("username");
        if (username == null || username.isBlank()) {
            return ResponseEntity.badRequest().body(CommonMethod.getReturnMessageError("切换账号不能为空！"));
        }
        Optional<User> userOptional = userRepository.findByUserName(username.trim());
        if (userOptional.isEmpty()) {
            return ResponseEntity.badRequest().body(CommonMethod.getReturnMessageError("目标账号不存在！"));
        }
    ...
    private String mapRoleTypeToAuthority(String roleType) {
        if (roleType == null) {
            return null;
        }
        return switch (roleType.trim()) {
            case "ADMIN", "ROLE_ADMIN" -> EUserType.ROLE_ADMIN.name();
            case "STUDENT", "ROLE_STUDENT" -> EUserType.ROLE_STUDENT.name();
            case "TEACHER", "ROLE_TEACHER" -> EUserType.ROLE_TEACHER.name();
            default -> null;
        };
    }
